...
if __name__ == '__main__':
    ...
    # Running another pip install will uninstall already installed packages
    os.chdir(os.path.pardir)
    standard_name = {'sofa': 'Sofa', 'pytorch': 'PyTorch'}
    for lower, standard in standard_name.items():
        if not PACKAGES[lower]:
            PACKAGES[lower] = find_spec(f'DeepPhysX_{standard}') is not None
    os.chdir('DeepPhysX')
