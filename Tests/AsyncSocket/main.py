...
env_config = BaseEnvironmentConfig(environment_class=Environment, number_of_thread=int(sys.argv[1]),
                                   socket_data_converter=BytesNumpyConverter)
env_manager = EnvironmentManager(environment_config=env_config, batch_size=2)
for i in range(3):
    data = env_manager.getData(batch_size=2, get_inputs=True, get_outputs=True, animate=True)
    print()
