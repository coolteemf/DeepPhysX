...
class Armadillo(BaseEnvironment):
    ...
    def create(self):
        ...
        sparse_grid_positions = self.sparse_grid.points().tolist()
        cell_corner = lambda x, y, z: sparse_grid_positions.index([x_sparse[x], y_sparse[y], z_sparse[z]])
        for zone in p_forces.zones:
            ...
            for i, node in enumerate(self.mesh_coarse.points()):
                if sphere(node, zone):
                    ...
                    ix = max(0, np.array(x_sparse < node[0], dtype=int).tolist().index(0) - 1)
                    iy = max(0, np.array(y_sparse < node[1], dtype=int).tolist().index(0) - 1)
                    iz = max(0, np.array(z_sparse < node[2], dtype=int).tolist().index(0) - 1)
                    self.g_areas[-1] += [cell_corner(ix, iy, iz),
                                         cell_corner(ix + 1, iy, iz),
                                         cell_corner(ix + 1, iy + 1, iz),
                                         cell_corner(ix, iy + 1, iz),
                                         cell_corner(ix, iy, iz + 1),
