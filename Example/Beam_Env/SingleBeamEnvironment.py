...
class SingleBeamEnvironment(SofaBaseEnvironment):

    def __init__(self, root_node, config, idx_instance=1, training=True):
        super(SingleBeamEnvironment, self).__init__(root_node, config, idx_instance)
        self.config = config
    ...
    def onAnimateBeginEvent(self, event):
        ...
        self.CFF.force.value = (5 / np.linalg.norm(F)) * F
