...
class UNetLayer(Module):
    ...
    def __init__(self,
        ...
        convolution_layer = Conv2d if config.nb_dims == 2 else Conv3d
        normalization_layer = BatchNorm2d if config.nb_dims == 2 else BatchNorm3d
        padding = 0 if config.border_mode == 'valid' else 1
        kernel_size = (3,) * config.nb_dims

        # Define the UNet layer: sequence of convolution, normalization and relu
        layers = [convolution_layer(in_channels=nb_input_channels,
...
class UNet(TorchNetwork):
    ...
    def __init__(self, config):
        ...
        up_convolution_layer = ConvTranspose2d if config.nb_dims == 2 else ConvTranspose3d
        last_convolution_layer = Conv2d if config.nb_dims == 2 else Conv3d
        self.max_pool = MaxPool2d(2) if config.nb_dims == 2 else MaxPool3d(2)
        self.skip_merge = config.skip_merge
        channels = config.nb_first_layer_channels
        up_kernel_size = (2,) * config.nb_dims
        final_kernel_size = (1,) * config.nb_dims

        # Define down layers : sequence of UNetLayer
        down_layers = [UNetLayer(nb_input_channels=config.nb_input_channels,
        ...
        self.architecture = EncoderDecoder(layers=layers, nb_encoding_layers=config.nb_steps + 1)
        ...
        self.finalLayer = last_convolution_layer(in_channels=channels,
